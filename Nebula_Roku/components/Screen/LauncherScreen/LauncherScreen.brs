...
sub onPlatformResponse()
  ' Diferir el procesamiento sin bloquear el hilo de SceneGraph.
    if validateStatusCode(m.apiRequestManager.statusCode) then
...
' Continúa procesando la respuesta de plataforma cuando finaliza la validación de imágenes.
sub __continuePlatformResponse()
  if m.platformResponseAwaitingImageValidation <> true then return
  m.platformResponseAwaitingImageValidation = false
...
' Dispara la regeneración de la información del usuario logueado
