...
sub onViewAll()
  ' Inicializo payload para resolver origen MainScreen o SearchScreen.
  viewAllData = invalid
  ' Si el stack actual está en MainScreen, tomo su payload viewAll.
  if m.StackOfScreens.Peek() = "MainScreen" then
    ...
    ' Si el stack actual está en SearchScreen, tomo su payload viewAll.
    viewAllData = m.SearchScreen.viewAll
    if viewAllData <> invalid then
      m.SearchScreen.viewAll = invalid
      __hideSearch()
    end if
  end if
  ' Si no existe payload, no hay navegación que ejecutar.
  if viewAllData = invalid then return
  ' Apilo pantalla ViewAll y la muestro con el payload recibido.
...
sub onStreamingPlayer()
  ...
  if m.StackOfScreens.Peek() = "MainScreen" then
    ...
    openGuide = m.SearchScreen.openGuide
    ' Limpiamos bandera para evitar retriggers en próximas navegaciones.
    m.SearchScreen.openGuide = false
...
sub __showSearch()
  ...
  ' Escucho salida viewAll para navegar a ViewAllScreen desde Search.
  m.SearchScreen.ObserveField("viewAll", "onViewAll")
end sub
...
sub __hideSearch()
  ...
  ' Dejo de escuchar salida de viewAll al ocultar Search.
  m.SearchScreen.unobserveField("viewAll")
  ...
  ' Limpio payload de viewAll para evitar retriggers.
  m.SearchScreen.viewAll = invalid
  ' Reseteo bandera openGuide usada para abrir player en modo guía.
  m.SearchScreen.openGuide = false
end sub
