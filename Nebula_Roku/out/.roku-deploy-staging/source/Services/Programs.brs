function urlProgramAction(key, id)
    ...
    return baseUrl + "/"+ m.global.apiVersions.V3 +"/Programs/" + key + "/" + id.toStr() + "/Actions"
end function

function urlProgramSummary( key, id, mainImageTypeId, imagesTypes)
    baseUrl = getServiceBaseUrl()
    return baseUrl + "/"+ m.global.apiVersions.V3 +"/Programs/" + key + "/" + id.toStr() + "/Summary?mit=" + mainImageTypeId.toStr() + "&bit=" + imagesTypes.toStr()
end function

function urlProgramById(key, id, mainImageTypeId, imagesTypes)
    baseUrl = getServiceBaseUrl()
    return baseUrl + "/"+ m.global.apiVersions.V3 +"/Programs/" + key + "/" + id.toStr() + "?mit=" + mainImageTypeId.toStr() + "&bit=" + imagesTypes.toStr()
end function
